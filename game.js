...
(function () {
  ...
  const BOSSES = [
    { name: 'IRON BEAST', color: 0x888888, hp: 800, scale: 2.8, arena: 0 },
    { name: 'THE NECRO CORE', color: 0x440066, hp: 900, scale: 2.2, arena: 1 },
    { name: 'INFERNO TITAN', color: 0xff4400, hp: 1000, scale: 3.0, arena: 2 },
    { name: 'VOID HUNTER', color: 0x220033, hp: 850, scale: 2.0, arena: 3 },
    { name: 'THE OVERLORD', color: 0xff00aa, hp: 1400, scale: 3.2, arena: 4 }
  ];
  ...
  function damageBoss(amount) {
    ...
    if (pct < 0.3 && boss.phase < 3) {
      boss.phase = 3;
      boss.atkCD = Math.max(0.9, boss.atkCD * 0.7);
      $('boss-phase').textContent = 'PHASE 3 - BERSERK';
    } else if (pct < 0.6 && boss.phase < 2) {
      boss.phase = 2;
      boss.atkCD = Math.max(1.1, boss.atkCD * 0.85);
      $('boss-phase').textContent = 'PHASE 2';
    }
    if (boss.hp <= 0) {
      boss.hp = 0;
      boss.alive = false;
      spawnParticle(boss.mesh.position, 0xffaa00, 50, 14, 1.2);
      camShake = 0.8;
      sfxVictory();
      score += 500 + (currentBoss + 1) * 200;
      endBoss();
    }
  }
  ...
  function endBoss() {
    ...
    saveGame();
    $('victory-stats').textContent = 'Score: ' + score + ' | Time: ' + Math.floor(elapsed) + 's | Combo peak tracked';
